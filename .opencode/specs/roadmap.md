...
### Decision Gate 2.1: decision evidence
...
Action:
- If `[x] Yes, all required tooling is available and compatible`: continue to Phase 3.
- If `[x] Partially, some tools or environment pieces are missing`: configure missing tooling and document setup steps.
- If `[x] No, critical tooling is missing`: stop and request prerequisite installation/availability before continuing.

### Decision Gate 2.2: version compatibility
...
### Decision Gate 3.1: implementation readiness
...
- If `[x] Partially, some implementation details are ambiguous`: resolve ambiguities against features/specs before coding.
- If `[x] No, implementation scope is not ready`: stop and refine feature definition first.

### Decision Gate 3.2: routing and ownership
...
Action:
- If `[x] All available`: continue build/deployment validation.
...
### Decision Gate 4.2: dependency policy
...
### Decision Gate 5.1: verification completeness
...
### Decision Gate 5.2: result status
...
### Decision Gate 6.1: documentation completeness
...
### Decision Gate 6.2: architecture and flow diagrams
Question: are required architecture/flow diagrams generated and aligned with final implementation?
- [ ] Yes
- [ ] Partial
- [ ] No
...
2. Decision-gate responses by phase with one selected checkbox per gate.
3. Agent ownership map by task and stack surface.
4. Implementation, build, and deployment log.
5. Test and security validation evidence.
6. Documentation and diagram evidence.
7. Final status with pass or blocker outcome.
...
- Build/deployment readiness is validated or blockers are documented.
